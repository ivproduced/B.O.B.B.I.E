...
class RAFamilyAgent(BaseFamilyAgent):
    family_id = "RA"
    controls_supported = ["RA-5"]

    def collect_evidence(self, control_id: str, context: dict[str, Any]) -> dict[str, Any]:
        if control_id.upper() != "RA-5":
            return {
                "status": "NOT_APPLICABLE",
                "findings": [f"{control_id} is not mapped to RA demo scope"],
                "recommendations": ["Use RA-5 for current RA family demo"],
                "risk_level": "LOW",
                "confidence_score": 1.0,
            }

        catalog = self.load_tagged_catalog(context)
        control = self.find_control(catalog, "ra-5")
        if control is None:
            return {
                "status": "FAIL",
                "findings": ["RA-5 was not found in the OSCAL catalog"],
        ...
        findings = []
        if not included:
            findings.append("RA-5 is not mapped to LOW/MODERATE/HIGH/PRIVACY baselines")

        evidence_eval = evaluate_control_effectiveness_with_objectives("RA-5", context)
        ...
        return {
            ...
            "findings": findings,
            "recommendations": [
                "Apply RA-5 CVSS SLA and KEV enrichment checks using vulnerability evidence",
                "Track open-age and exploitability in remediation prioritization",
            ...
            "evidence": {
                ...
                "baselines": baselines,
                "control_evidence": context.get("control_evidence", {}).get("RA-5", {}),
